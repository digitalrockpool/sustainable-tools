...
<form method="post" name="edit" id="<?php echo $update_charity ?>" class="needs-validation" novalidate>
  ...
  <div id="repeater-field">
    <div class="entry row g-1 mb-1"> <?php
      ...
      <div class="<?php if( $action == 'edit' ) : ?>col-md-4<?php else : ?>col-md-5<?php endif; ?>">
        <?php if( $action == 'edit' ) : ?><label for="edit-donee-location">Donee Location<sup class="text-danger">*</sup></label><?php endif; ?>
        <select class="form-select" id="edit-donee-location" name="edit-donee-location[]" required>
          <?php if( $action == 'add' ) : ?><option value="">Select Donee Location</option><?php endif; ?>
          <option value="0" <?php if( $action == 'edit' ) : echo 'selected'; else : echo ''; endif; ?>>Unknown Location</option> <?php
      ...
      if( $tag_id == 66 ) : ?>
        ...
        <div class="<?php if( $action == 'edit' ) : ?>col-md-4<?php else : ?>col-md-3<?php endif; ?>">
          <?php if( $action == 'edit' ) : ?><label for="edit-duration">Staff Time Donated<sup class="text-danger">*</sup></label><?php endif; ?>
        ...
        <div class="<?php if( $action == 'edit' ) : ?>col-md-4<?php else : ?>col-md-3<?php endif; ?>">
          <?php if( $action == 'edit' ) : ?><label for="edit-value-type">Value Type<sup class="text-danger">*</sup></label><?php endif; ?>
      ...
      <div class="<?php if( $action == 'edit' ) : ?>col-md-4<?php else : ?>col-md-3<?php endif; ?>">
        <?php if( $action == 'edit' ) : ?><label for="edit-amount"><?php if( $tag_id == 66 ) : echo 'Cash Equivalent'; else : echo 'Amount / Cash Equivalent'; endif; ?><sup class="text-danger">*</sup></label><?php endif; ?>
  ...
  <div class="row">
    <div class="col-12 mb-3"><button class="btn btn-primary" type="submit" name="<?php echo $update_charity ?>"><?php if( $action == 'edit' ) : echo 'Update'; else : echo 'Add'; endif; echo ' '.str_replace( '-', ' ', $tag_url ); ?></button></div>
  </div>
</form> <?php
